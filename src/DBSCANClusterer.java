...
public class DBSCANClusterer<V> {
     ...
     */
    public DBSCANClusterer(final Collection<V> inputValues, int minNumElements, double maxDistance,
            DistanceMetric<V> metric) throws DBSCANClusteringException {
    ...
    public Clusters converttoClusters(ArrayList<ArrayList<TripRecord>> triplist){
        Clusters clusters = new Clusters();
        for(ArrayList<TripRecord> trip: triplist){
    ...
    public ArrayList<ArrayList<V>> performClustering() throws DBSCANClusteringException {
        ...
        if (inputValues.size() < 2) {
            throw new DBSCANClusteringException(
                    "DBSCAN: Less than two input values cannot be clustered. Number of input values: "
                            + inputValues.size());
        }

        if (epsilon < 0) {
            throw new DBSCANClusteringException(
                    "DBSCAN: Maximum distance of input values cannot be negative. Current value: " + epsilon);
        }

        if (minimumNumberOfClusterMembers < 2) {
            throw new DBSCANClusteringException(
                    "DBSCAN: Clusters with less than 2 members don't make sense. Current value: "
